...
public class Player__ : MonoBehaviour
{
    ...
    private bool isLockon;      // ロックオンフラグ
    ...
    private void FixedUpdate()
    {
        ...
        //- 無敵フラグが無効なら
        if (!isInvRoll && !isInvStep && !isJust && !isDamage && !isDeath/* && !isInvJust*/)
        {
    ...
    void Update()
    {
        ...
        if (isLockon)
        {
            ...
            {
                camera.GetComponent<ChangeCamera>().CamChange(); // カメラ変更
                isLockon = false; // ロックオンフラグ無効化
            }
        }
        else if (isMove) // 移動フラグが有効なら
        {
            //- プレイヤーの向き変更処理
    ...
    private void OnMove(InputAction.CallbackContext context)
    {
        ...
        if (!isKeyBoard)
        {
            ...
            //Debug.Log("スティック入力：" + stickMove);
            //- カメラの方向を取得
            Vector3 camForward = camera.transform.forward;
            Vector3 camRight = camera.transform.right;
            //- 移動方向を計算
            Vector3 moveDir = (camForward * stickMove.y + camRight * stickMove.x).normalized;
            //- 移動量を格納
    ...
    {
        if (!isInvStep && !isInvRoll && eneBarCS.energy >= eneBarCS.SubStep)
        {
            // UIの色を変える
    ...
    private void OnStep(InputAction.CallbackContext context)
    {
        ...
        if (/*!isDash && */!isInvRoll && !isInvStep && eneBarCS.energy >= eneBarCS.SubStep)
        {
            Debug.Log("ステップ");
            //- UIの色を変える
            ...
            //- アニメーション再生
            playerAnim.SetBool("isStep", true);
    ...
    public void Damage(int damage)
    {
        ...
        if (!isInvRoll && !isInvStep && !isInvJust && !isDamage && !isDeath)
        {
            ...
            {
                // アニメーション再生
                playerAnim.SetTrigger("trDamage");
                // ダメージフラグ有効化
                isDamage = true;
            }
        }
    ...
    private void InvCount()
    {
        ...
        if (isInvRoll)
        {
            ...
            if (curInvTime > rollInvTime)
            {
                ...
                //rollUI.GetComponent<PushUI>().Release();   //- UIの色を戻す
                Debug.Log("無敵解除(回避)");
            }
        }
        //- ステップの無敵フラグが有効なら
        if (isInvStep)
        {
            ...
            {
                playerAnim.SetBool("isStep", false);
            ...
            if (curJustTime <= justAcceptTime) // 現在の受け付け時間が最大受け付け時間以下なら
            {
                ...
                if (lockonEnemy && enemyBlt)
                {
                    if (lockonEnemy.name == enemyBlt.GetComponent<EnemyBullet>().GetEnemyName())
                    {
                        ...
                        Time.timeScale = 0.7f; // スローモーションにするためタイムスケール変更
                        Debug.Log("ジャスト回避発動");
    ...
    private void JustAttack()
    {
        ...
        if (autoAtkEnemy)
        {
            ...
            //- 弾の発射ベクトルを敵に向かうよう計算
            bulletDir = autoAtkEnemy.transform.position - Gun.transform.position;
            //- 計算したベクトルを弾に格納
            Bullet.GetComponent<PlayerBullet>().SetBullet(
                bulletDir.normalized.x,
                bulletDir.normalized.y,
                bulletDir.normalized.z);
            Bullet.GetComponent<PlayerBullet>().SetPlayerPos(autoAtkEnemy.transform.position);
            isJust = false; // ジャスト回避フラグ無効化
            isInvJust = false; // ジャスト回避の無敵フラグ無効化
            isJustAccept = false; // ジャスト回避受け付けフラグ無効化
            isInvStep = false;
